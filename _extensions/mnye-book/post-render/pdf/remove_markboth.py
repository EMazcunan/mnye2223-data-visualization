...
def lines_without_markboth(tex_file):
    lines = []
...
def remove_markboth(tex_file):
    lines = lines_without_markboth(tex_file)
    utils.replace_lines(tex_file, lines)

if ".pdf" in os.getenv("QUARTO_PROJECT_OUTPUT_FILES"):
    tex_file = utils.tex_file()
    remove_markboth(tex_file) # path relativo al proyecto, no a este script